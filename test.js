...
class SubtitleConvertor
{
     ...
     */
    convertAssFormatsToCss(styleParts, ssaStyleFormat, assVideoHeight, headerData = true)
    {
        let i = 0;
        let generatedStyle = '';
...
fs.readFile("./track3_eng.ass", "utf8", (err, data) => {
    if (err) {
        console.error(err);
        return;
    }

    const converter = new SubtitleConvertor();
    converter.subtitleText = data
    convertedText = converter.convertFromAss('srt');

    fs.writeFile('./test.srt', convertedText, (err) => {
        if (err) {
            console.log(err);
        }
    });
